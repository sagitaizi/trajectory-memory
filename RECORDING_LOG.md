...
| wall_target/loop_break_01 | 25.0 s | 1047 k/s | deviation clip; highest rate, 26M events; framed OK. **Labelled** — diagonal sweep to ~10.5 s, break with an excursion to x = 366, then a *horizontal* sweep from ~13 s: the deviation is a change of direction. Lap period 1.0 s on this take, so 0.2 s marks are only 5 per lap — turnarounds undershoot by ~15–25 px. Fine for detection; use loop_01 for prediction error, or densify to 0.1 s if needed |
