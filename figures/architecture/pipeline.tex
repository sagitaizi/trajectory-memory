...
\definecolor{mem}{HTML}{C98500}
\definecolor{memfill}{HTML}{FBF0D9}
...
\begin{scope}[shift={(104,11)}]
  ...
  \draw[->, mem, line width=0.9pt] (30:5.2) arc[start angle=30, end angle=95, radius=5.2];
  \draw[muted, line width=0.4pt] (0,0) -- (30:5.2);
  \fill[ink] (30:5.2) circle (0.75);
  \draw[mem, fill=white, line width=0.7pt] (95:5.2) circle (0.75);
  ...
  \begin{scope}[shift={(14,0)}]
    \foreach \a in {0,20,...,340} {
      ...
      \filldraw[fill=mem!\act!white, draw=mem!60!faint, line width=0.3pt] (\a:5.2) circle (0.8);
    }
    \node[lab] at (0,-9.3) {phase\\[-1pt]cells};
  \end{scope}
  \draw[flow] (5.8,0) -- (7.9,0);
  \begin{scope}[shift={(32,0)}]
    \filldraw[fill=memfill, draw=mem, line width=0.6pt, rounded corners=1.5pt] (-5,-5) rectangle (5,5);
    \draw[mem, line width=0.9pt] (-3.8,1.8) .. controls (-1.5,-2.2) and (1.5,-2.2) .. (3.8,1.8);
...
\node[sub, anchor=north, align=left] at (out.south) {%
  {\color{mem}\textbf{---}} remembered path\\
  $\circ$ $p(t)$\quad {\color{mem}$\bullet$} $\hat{p}(t{+}h)$};
