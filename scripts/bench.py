...
def make_overlays(clip, localiser, window_us: int, horizon_s: float, warmup_s: float) -> list[LiveOverlay]:
...
class Panels:
    ...
    def frame(self, t: float, note: str = "") -> np.ndarray:
        ...
        panels = []
        for k, ov in enumerate(self.overlays):
            step = ov.at(mid)
...
def main(argv=None) -> None:
    ...
    for e in entries:
        ...
        overlays = make_overlays(clip, localiser, args.window_us, args.horizon, args.warmup)
